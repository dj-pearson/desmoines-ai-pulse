...
# The App Clip is a separate target/module and can't see the main target's
# generated file, so it gets its own copy (IOS-AUDIT-REL-003).
CLIP_OUTPUT_FILE="$PROJECT_DIR/DesMoinesInsiderClip/ClipSecrets.generated.swift"

# Writes a GeneratedSecrets enum to $1 with the resolved values.
write_secrets_file() {
    cat > "$1" << SWIFT
...
if [ -z "$SUPABASE_URL_VALUE" ] && [ -z "$SUPABASE_ANON_KEY_VALUE" ] && [ -f "$OUTPUT_FILE" ]; then
    ...
    if [ -n "$EXISTING_URL" ] && [ "$EXISTING_URL" != "" ]; then
        EXISTING_KEY=$(grep 'supabaseAnonKey' "$OUTPUT_FILE" 2>/dev/null | sed 's/.*= "\(.*\)"/\1/' || true)
        # Recover the values so the Clip copy can be (re)generated to match,
        # without clobbering the main file's CI-injected secrets.
        SUPABASE_URL_VALUE="$EXISTING_URL"
        SUPABASE_ANON_KEY_VALUE="$EXISTING_KEY"
        write_secrets_file "$CLIP_OUTPUT_FILE"
        echo "✅ Secrets.generated.swift already has values — skipping main, synced Clip copy"
        exit 0
    fi
fi
...
# Generate the Swift files (main target + App Clip copy).
write_secrets_file "$OUTPUT_FILE"
write_secrets_file "$CLIP_OUTPUT_FILE"

echo "✅ Generated Secrets.generated.swift (+ Clip copy)"
