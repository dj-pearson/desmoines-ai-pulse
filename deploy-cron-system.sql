-- Deploy Cron System for Automated Scraping
-- Run this script in your Supabase SQL Editor to set up the automated scraping system
...
DECLARE
  ...
  function_url TEXT;
BEGIN
  -- Get the Supabase project URL (replace with your actual URL)
...
-- Verify the setup
SELECT 'Cron system deployed successfully!' as status;
SELECT 'Active cron jobs:' as info;
SELECT jobname, schedule, active FROM cron.job WHERE jobname = 'scraping-jobs-runner';

SELECT 'Scraping jobs with next_run times:' as info;
SELECT id, name, next_run, config->>'schedule' as schedule 
