...
const googleSearchApiKey =
  Deno.env.get("GOOGLE_SEARCH_API") || Deno.env.get("GOOGLE_PROGRAMMATIC_KEY");
const googleSearchEngineId =
  Deno.env.get("GOOGLE_SEARCH_ENGINE_ID") || "a67b454ea60fc4b35";
...
async function searchWithGoogle(query: string): Promise<SearchResult[]> {
  ...
  console.log(
    "Google Search URL:",
    searchUrl.replace(googleSearchApiKey, "[API_KEY_HIDDEN]"),
  );
  ...
  console.log(
    "Search info:",
    JSON.stringify(data.searchInformation || {}, null, 2),
  );
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Batch enhancing ${eventIds.length} events with fields: ${fields.join(", ")}`,
    );
    console.log("Environment check - Claude API Key exists:", !!claudeApiKey);
    console.log(
      "Environment check - Google Search API Key exists:",
      !!googleSearchApiKey,
    );
    ...
    // Get the events from the database - only future events to avoid processing past events
    const now = new Date().toISOString();
    const { data: events, error: eventsError } = await supabase
      .from("events")
      .select("*")
      .in("id", eventIds)
      .gte("date", now); // Only process future events

    if (eventsError) {
      throw eventsError;
    }

    const skippedPast = eventIds.length - (events?.length || 0);
    if (skippedPast > 0) {
      console.log(
        `⏭️ Skipped ${skippedPast} past events (only processing future events)`,
      );
    }
    ...
    for (const event of events) {
      try {
        ...
        if (
          event.venue &&
          !event.title.toLowerCase().includes(event.venue.toLowerCase())
        ) {
        ...
        if (searchResults.length === 0 && searchQuery !== event.title) {
          console.log(
            `No results found, trying simplified search: ${event.title}`,
          );
          searchResults = await searchWithGoogle(event.title);
          console.log(
            `Simplified search results count: ${searchResults.length}`,
          );
        }
        ...
        const improvements = await analyzeWithClaude(
          searchResults,
          dbFields,
          currentEventData,
        );
        ...
        console.log(
          `Claude analysis complete for event ${event.id}:`,
