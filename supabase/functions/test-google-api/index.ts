...
serve(async (req) => {
  ...
  try {
    ...
    const geocodeUrl = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(testLocation)}&key=${GOOGLE_API_KEY}`;
    ...
    return new Response(
      JSON.stringify({
        ...
        geocoding: {
          ...
          coordinates: geocodeData.results?.length > 0 ? geocodeData.results[0].geometry.location : null
        },
        places: placesTest,
