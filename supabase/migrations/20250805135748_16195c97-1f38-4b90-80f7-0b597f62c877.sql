...
SELECT 
    e.*,
    COALESCE(COUNT(cm.id), 0) as view_count
FROM events e
LEFT JOIN content_metrics cm ON cm.content_id = e.id 
    AND cm.content_type = 'event'
    AND cm.metric_type = 'view'
    AND cm.date >= CURRENT_DATE - INTERVAL '7 days'
WHERE e.date >= CURRENT_DATE
GROUP BY e.id
ORDER BY view_count DESC, e.date ASC
LIMIT 50;

-- Index on materialized view
CREATE UNIQUE INDEX IF NOT EXISTS idx_trending_events_id ON trending_events(id);
CREATE INDEX IF NOT EXISTS idx_trending_events_view_count ON trending_events(view_count DESC);
