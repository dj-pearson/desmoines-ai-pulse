...
-- Update the restaurants table to include view count tracking
ALTER TABLE public.restaurants 
ADD COLUMN IF NOT EXISTS view_count INTEGER DEFAULT 0;

-- Add updated_at trigger for popularity_score
CREATE OR REPLACE FUNCTION update_restaurant_popularity()
RETURNS TRIGGER AS $$
BEGIN
  -- Recalculate popularity score when rating or featured status changes
  IF OLD.rating IS DISTINCT FROM NEW.rating OR OLD.is_featured IS DISTINCT FROM NEW.is_featured THEN
    NEW.popularity_score = calculate_restaurant_popularity_score(
      NEW.rating,
      NEW.is_featured,
...
-- Create trigger
DROP TRIGGER IF EXISTS trigger_update_restaurant_popularity ON public.restaurants;
CREATE TRIGGER trigger_update_restaurant_popularity
  BEFORE UPDATE ON public.restaurants
  FOR EACH ROW
  EXECUTE FUNCTION update_restaurant_popularity();
