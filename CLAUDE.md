...
| Sponsored/featured listings in browse lists | `@/lib/sponsoredListings` — `isActivelySponsored(item)` (expiry-aware: `is_sponsored` true AND `sponsored_until` absent/future) gates a card's amber ring + `<SponsoredBadge/>` (`@/components/SponsoredBadge`, FTC-labeled); `promoteSponsored(items, MAX_SPONSORED_AT_FRONT=2)` floats ≤2 sponsored to the front (organic order otherwise untouched) — call at page level AFTER sort/filter, BEFORE pagination. Track via `useSponsoredTracking(active,type,id)` (viewability impression + click, `'sponsored_listing'` inventory class in `@/lib/tracking`). Shown to all tiers (content, not an ad gate). Browse routes use distinct cards: /events→`SocialEventCard`, /restaurants→`RestaurantCard`, /attractions→`AttractionListCard`. WEB-FEAT-005 |
| Recently-viewed + data-driven home order | `@/lib/recentlyViewed` (versioned safeStorage external store like `guestFavorites`; `recordView`/`removeRecentlyViewed`, cap 20 + 30-day prune, `getRecentlyViewedSnapshot`/`subscribeRecentlyViewed` for `useSyncExternalStore`, `dominantRecentType`) + `useRecentlyViewed` (back-compat `addToRecentlyViewed(event)`; signed-in write-through + once-per-user merge to the `recently_viewed` server table — owner-RLS, not in generated types → `as never` casts). Record a view on each detail page (event/restaurant/attraction) via a `recordedViewRef` once-guard. Multi-type UI: `RecentlyViewed` (`variant="rail"` gated `minEntries`>=3 / `variant="list"` for dashboard). Home ordering: `@/lib/homeSectionOrder` (`orderHomeSections` pure + deterministic — permutes only typed sections, pins `neutral`; `cohortFor`) + `useHomeSectionOrder` (signals read synchronously → no CLS; logs cohort via `@/lib/personalizationAnalytics`). WEB-FEAT-007 |
| Social-share OG image (detail pages) | `@/lib/ogImage` — `getOgImageUrl(type,id)` builds the `og-image` edge fn URL (`${VITE_SUPABASE_URL}/functions/v1/og-image/<type>/<id>`); pass as `SEOHead`'s `ogImageUrl` prop (→ `og:image`+`twitter:image`; `imageUrl` stays the schema/fallback image). When a page renders BOTH a domain SEO comp AND `SEOHead`, react-helmet-async keeps the LAST `og:image` in render order — wire whichever renders last (EventDetails→`EnhancedEventSEO`; Attraction/Article/Restaurant→`SEOHead`). The edge fn (satori→resvg-wasm branded-gradient PNG) self-heals: 302s to the item image (via `image-transform`) or static `/og-image.png` on any failure. WEB-FEAT-008 |
...
| Trip Planner web UI (quota / reorder / map / share / calendar) | Backend (`generate-itinerary`, `trip_plans`, `get_trip_itinerary` RPC, share codes) is shared with iOS — web is UI-only. `useTripPlanner` exposes `getTripQuota(tier)` (from `TRIP_PLANNER_MONTHLY_QUOTA` free 0/insider 5/vip -1 — MUST mirror the server constant in generate-itinerary; counts `tripsThisMonth` = ai_generated rows since start of UTC month, same window the edge fn counts) + `reorderItems` (batch order_index). Itinerary rendering is the shared `@/components/trip/ItineraryDays` (editable→up/down reorder via `onMoveItem` with optimistic local swap; read-only for the public page); the stop map is `@/components/trip/TripMapPreview` (react-leaflet; batch-fetches lat/lng per content type via `.in('id',ids)` — the RPC's `content_details` has NO coords, so no migration) lazy()+`RevealOnVisible` so leaflet stays off the page chunk (bundles into the leaflet group chunk). Share = make public then `TripShareDialog` (copy + `navigator.share`); the link resolves at the public `SharedTrip` page (`/trips/shared/:code`, robots noindex via Helmet). Calendar = `@/lib/tripCalendar` (`downloadTripIcs` whole-trip/per-day, `getTripGoogleCalendarUrl`; Central wall-time→UTC via `centralTimeToUtc`) surfaced by `TripCalendarMenu`. At-quota or a server quota_exceeded/upgrade_required body → `PaywallModal context="trip_planner"`. WEB-FEAT-011 |
| Star ratings + reviews (+ photos) on a detail page | `<RatingSystem contentType contentId showReviews />` (`@/components/RatingSystem`) — full composer (gated by `PremiumGate feature="write_reviews"`), reviews list, helpful votes, report. Reuses `@/components/reviews/{StarRating,ReviewComposer,ReviewPhotoGrid}` + `useRatings` (`submitRating(rating,text?,photoUrls?)`, `reportReview(id)`). Photos: `@/lib/reviewPhotos` (`compressReviewPhoto`→JPEG <500KB, `uploadReviewPhotos`→owner-scoped `review-photos` bucket). Submits go through WEB-AUTO-009 `moderate-content` (clean publishes instantly; author sees own pending via the `.or(approved,user_id)` read filter). Report → `moderate-content {action:'report'}` re-flags into the moderation queue at 3 distinct reporters. WEB-FEAT-010 |
