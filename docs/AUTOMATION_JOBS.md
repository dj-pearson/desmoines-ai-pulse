...
| **`saved-search-alerts-nightly`** | `0 13 * * *` | fn `saved-search-alerts` | **Saved-search digest: for each alert-enabled `saved_searches` row, finds upcoming events created since its `last_alerted_at` watermark matching the saved query+category, groups per user, sends ONE consolidated email (deep links back to the filtered view + each event); respects unsubscribe, zero matches = no email; per-tier funnel counts in metadata (WEB-FEAT-003)** | ✅ |
