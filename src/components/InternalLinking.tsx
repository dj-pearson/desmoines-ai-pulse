...
export function InternalLinking({
  ...
  const { data: featuredContent } = useQuery({
    ...
    queryFn: async () => {
      ...
      log.debug('Fetching featured content', { action: 'fetchFeaturedContent', metadata: { dateGte: today } });
      ...
      log.debug('Featured events found', { action: 'fetchFeaturedContent', metadata: { count: eventsResult.data?.length } });
      return {
