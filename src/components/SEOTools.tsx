...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/" xmlns:atom="http://www.w3.org/2005/Atom">
  ...
  const handleGenerate = async (generatorId: string) => {
    ...
    try {
      ...
      log.error(`Generation error for ${generatorId}`, { action: 'handleGenerate', metadata: { error, generatorId } });
      toast({
  ...
  const handleSeoGeneration = async (contentType: 'event' | 'restaurant') => {
    ...
    try {
      ...
      log.error('SEO generation error', { action: 'handleSeoGeneration', metadata: { error } });
      toast({
