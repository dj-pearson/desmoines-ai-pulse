  ...
  useEffect(() => {
    if (open && factorId && !challengeId) {
      createChallenge(factorId)
        .then(setChallengeId)
        .catch((error) => {
          log.error('Failed to create MFA challenge', { action: 'createChallenge', metadata: { error } });
      ...
      setTimeout(() => {
        if (challengeId && factorId) {
          verifyChallenge(factorId, challengeId, cleanedValue)
            ...
            .catch((error) => {
              log.error('MFA verification failed', { action: 'handleCodeChange', metadata: { error } });
              setAttempts(prev => prev + 1);
              setVerificationCode('');
