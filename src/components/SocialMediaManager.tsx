...
import { Loader2, Plus, Send, Edit, Trash2, Globe, Calendar, Bot, ExternalLink } from 'lucide-react';
...
const SocialMediaManager = () => {
  ...
  const handleGeneratePost = async (contentType: 'event' | 'restaurant', subjectType: string) => {
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Social Media Manager</h2>
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                              ...
                              <Badge variant="outline">{post.subject_type.replace('_', ' ')}</Badge>
                              <Badge variant="outline">{post.platform_type}</Badge>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          {post.scheduled_for && (
                            <div className="flex items-center">
                              ...
                              Scheduled: {format(new Date(post.scheduled_for), 'MMM dd, HH:mm')}
                            </div>
                          )}
                          {post.posted_at && (
                            <div className="flex items-center">
                              <Calendar className="h-3 w-3 mr-1" />
                              Posted: {format(new Date(post.posted_at), 'MMM dd, HH:mm')}
                            </div>
                          )}
                        </div>
                        <span>Created: {format(new Date(post.created_at), 'MMM dd, HH:mm')}</span>
                      </div>
                    </Card>
                  ))}
                </div>
        ...
        <TabsContent value="generate" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                The AI will automatically select content that hasn't been featured recently and generate both Twitter/Threads (under 200 chars) and Facebook/LinkedIn (longer format) versions.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    <h4 className="font-semibold mb-2">Twitter/Threads Format</h4>
                  ...
                  <div>
                    <h4 className="font-semibold mb-2">Facebook/LinkedIn Format</h4>
                    <ul className="text-sm text-muted-foreground space-y-1">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Configure webhooks to automatically distribute posts to social media platforms
                </CardDescription>
              </div>
