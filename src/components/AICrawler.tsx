...
const AICrawler: React.FC = () => {
  ...
  const handleSourceSelect = (source: { url: string; category: string }) => {
    setUrl(source.url);
  ...
  const handleCrawl = async () => {
    ...
    try {
      log.info('crawl', 'Starting AI crawl', { url, category });

      const { data, error } = await supabase.functions.invoke(
        "ingest/firecrawl-scraper",
        {
          body: { url, category },
        }
      );

      log.debug('crawl', 'AI crawl response', { data, error });
      ...
      log.error('crawl', 'AI crawl error', { data: error });
      setResult({
        success: false,
        totalFound: 0,
        futureEvents: 0,
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-xs text-gray-600">
              <ul className="list-disc list-inside space-y-1">
                <li>
                  Now works with JavaScript-heavy sites! Waits for content to
                  load dynamically
                </li>
                <li>
                ...
                <li>
                  The AI will automatically detect and skip duplicate entries
                </li>
                <li>
                  Large websites may take 30-60 seconds to process completely
                </li>
                <li>
                  Try specific pages (like /events or /menu) rather than
                  homepage for better results
                </li>
              </ul>
            </CardContent>
          </Card>
