...
export function useNativeShare(): UseNativeShareReturn {
  ...
  const copyToClipboard = useCallback(async (text: string) => {
    try {
      ...
      log.error('Copy to clipboard failed', { action: 'copyToClipboard', metadata: { error } });
      toast.error('Failed to copy to clipboard');
    }
  }, []);
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.name === 'AbortError') {
          log.debug('Share cancelled by user', { action: 'share' });
        } else {
          log.error('Share failed', { action: 'share', metadata: { error } });
