...
const logger = createLogger('useEventReminders');
...
export function useEventReminders(eventId: string) {
  ...
  const { data: remindersData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        logger.error('fetchReminders', 'Error fetching reminders', { error });
        throw error;
      }
  ...
  const toggleReminderMutation = useMutation({
    ...
    onError: (error: any) => {
      logger.error('toggleReminder', 'Error toggling reminder', { error });
      toast({
