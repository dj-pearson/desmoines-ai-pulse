...
export function useCampaignRenewal() {
  ...
  const renewCampaign = async (
    ...
    try {
      ...
      log.error('renewCampaign', 'Error renewing campaign', { error: err });
      toast({
        variant: "destructive",
  ...
  const enableAutoRenew = async (campaignId: string): Promise<boolean> => {
    try {
      ...
      log.error('enableAutoRenew', 'Error enabling auto-renew', { error: err });
      toast({
        variant: "destructive",
  ...
  const disableAutoRenew = async (campaignId: string): Promise<boolean> => {
    try {
      ...
      log.error('disableAutoRenew', 'Error disabling auto-renew', { error: err });
      toast({
        variant: "destructive",
