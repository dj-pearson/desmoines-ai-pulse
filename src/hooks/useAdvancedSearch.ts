...
export function useAdvancedSearch() {
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          log.debug('Location access denied', { action: 'getUserLocation', metadata: { error } });
        }
      );
    }
  }, []);
  ...
  const loadSavedSearches = useCallback(async () => {
    ...
    try {
      ...
      log.error('Error loading saved searches', { action: 'loadSavedSearches', metadata: { error } });
      setSavedSearches([]);
    }
  }, [user]);
  ...
  const performSearch = useCallback(async (searchFilters: AdvancedSearchFilters) => {
    ...
    try {
      ...
      log.error('Search error', { action: 'performSearch', metadata: { error } });
  ...
  const saveSearch = async (name: string, searchFilters: AdvancedSearchFilters) => {
    ...
    try {
      ...
      log.error('Error saving search', { action: 'saveSearch', metadata: { error } });
  ...
  const loadSearch = async (search: SavedSearch) => {
    try {
      ...
      log.error('Error loading search', { action: 'loadSearch', metadata: { error } });
      setFilters(search.filters);
      await performSearch(search.filters);
    }
  };
  ...
  const deleteSearch = async (searchId: string) => {
    try {
      ...
      log.error('Error deleting search', { action: 'deleteSearch', metadata: { error } });
