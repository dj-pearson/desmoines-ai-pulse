...
const logger = createLogger('useEventRecommendations');
      ...
      try {
        ...
        if (error) {
          ...
          if (error.code !== '42883' && error.code !== '42804' && !error.message?.includes('does not exist')) {
            logger.error('personalizedRecommendations', 'Error fetching personalized recommendations', { error });
          }
    ...
    queryFn: async () => {
      try {
        ...
        if (error) {
          // Only log unexpected errors (not function not found or schema errors)
          if (error.code !== '42883' && error.code !== '42804' && !error.message?.includes('does not exist')) {
            logger.error('trendingEvents', 'Error fetching trending events', { error });
          }
          return [] as RecommendedEvent[];
        }
