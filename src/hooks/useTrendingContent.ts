...
export function useTrending(options: TrendingOptions = {}) {
  ...
  const fetchTrendingItems = async () => {
    try {
      ...
      // (.in('id', ids)) instead of a query per trending item (N+1). With at
      // most 4 content types this is <= 4 round-trips regardless of `limit`.
        ...
        await Promise.all(
          Array.from(idsByType.entries()).map(async ([type, ids]) => {
            try {
              ...
              console.log(`Could not fetch content for ${type}`);
            }
          })
        );
      }
