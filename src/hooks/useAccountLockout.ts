...
export function useAccountLockout() {
  ...
  const checkLockoutMutation = useMutation({
    mutationFn: async (email: string): Promise<LockoutStatus> => {
      ...
      if (error) {
        log.error('Error checking lockout status', { action: 'checkLockout', metadata: { error } });
        throw error;
      }
  ...
  const recordAttemptMutation = useMutation({
    mutationFn: async ({
      ...
      if (error) {
        log.error('Error recording login attempt', { action: 'recordAttempt', metadata: { error } });
        throw error;
      }
  ...
  const { data: lockoutSettings } = useQuery<AccountLockoutSettings | null>({
    ...
    queryFn: async () => {
      ...
      if (error) {
        log.error('Error fetching lockout settings', { action: 'fetchSettings', metadata: { error } });
        return null;
      }
