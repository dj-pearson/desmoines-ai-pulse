...
export function useAdvancedAnalytics() {
      ...
      setSessionMetrics((prev) => ({
        ...
        searchQueries:
          interaction.interactionType === "search"
            ? prev.searchQueries + 1
            : prev.searchQueries,
        filtersUsed:
          interaction.interactionType === "filter"
            ? prev.filtersUsed + 1
            : prev.filtersUsed,
        contentEngaged: ["view", "click", "share", "bookmark"].includes(
          interaction.interactionType
        )
          ? prev.contentEngaged + 1
          : prev.contentEngaged,
      }));
  ...
  const derivePreference = useCallback((interaction: UserInteraction) => {
    ...
    if (
      interaction.contentType === "restaurant" &&
      interaction.duration &&
    ...
    if (interaction.interactionType === "filter" && interaction.value) {
      ...
      const preference: UserPreference = {
        category:
          interaction.elementType === "filter_dropdown"
            ? "cuisine"
            : "location", // Simplified
  ...
  const flushQueuedData = useCallback(async () => {
    if (
      interactionQueue.current.length === 0 &&
      preferenceQueue.current.length === 0
    )
      return;
    ...
    try {
      ...
      if (interactionQueue.current.length > 0) {
        ...
        await supabase
          .from("user_interactions_enhanced")
