...
import { useEventLanding } from '@/hooks/useEventLanding';
...
/** Up to six events this weekend. Hides itself when there are none or the read fails. */
function ThisWeekendModule() {
  const { data, isLoading, isError, error } = useEventLanding({
    key: { landing: 'visitors-guide' },
    window: 'this-weekend',
    limit: WEEKEND_LIMIT,
  });
        ...
        <div className="container mx-auto px-4 py-8 max-w-4xl">
          ...
          >
            <p className="text-muted-foreground max-w-prose">
              Hungry after a flight or a show? This list is worked out from each restaurant&apos;s posted hours
              in Central time, so it changes through the day.
            </p>
          </GuideModule>
          ...
          >
            <p className="text-muted-foreground max-w-prose">
              The airport is about 5 miles from downtown. Downtown garages, the skywalk, DART fares and bike
              share are on one page, with where each fact came from.
            </p>
          </GuideModule>
